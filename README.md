...
- No need to copy/move any files; just start `distributable\START HERE - Quality Barony.bat`.
...
- <code>Quality synchronization uses Barony's common packet transport for direct connections, Steam P2P, and Crossplay. Clients repeat a slot-based handshake until the host's current floor is confirmed, then receive chest states and follower data.</code>
- <code>[untested]</code> <code>A hostile that is inside the viewing player's or ghost's real camera view, illuminated in that viewer's player-specific lightmap, and not blocked by a wall or closed sight-blocking entity appears as a solid purple dot for that viewer. This uses only ordinary local game state and follows darkness, sneaking vision, carried lights, full invisibility, dithered visibility, and telepathy. A sighting disappears when that viewer can no longer see the creature; vanilla minimap-only detection effects remain separate.</code>
...
- <code>That exit dialog also shows x hostiles / y neutrals alive for the viewing player; followers are excluded. Hosts and single-player use authoritative HP, while multiplayer clients show a best-effort estimate from active monster entities and locally available friendliness state. Once that player's calculated hostile count reaches two or fewer, all calculated living hostiles and neutrals remain visible as purple dots until the next floor, even if the count later rises. A ghost entering the exit's 3×3 tile neighborhood triggers the same full reveal and sees the calculated count persistently below the minimap until leaving the area.</code>
...
<table>
  ...
  <tbody>
    ...
    <tr><td><img src="distributable/minimap/purple-dot.svg" width="18" alt="Purple dot"></td><td><img src="distributable/minimap/purple-dot.svg" width="18" alt="Purple dot"></td><td>Detected unit</td><td>Persists while detected by vanilla effects. <code>Quality also uses it for hostiles currently visible in the viewing player's camera and player-specific lightmap, and for that player's latched final hostile/neutral reveal.</code></td></tr>
